...
def run(args):
	...
	center_attribute_assignment = np.ndarray((kmeans.n_components, n_targets), dtype=bool)
	...
	for attr in range(n_targets):
		...
		center_yes = np.ndarray((kmeans.n_components,), dtype=int)
		center_no = np.ndarray((kmeans.n_components,), dtype=int)
...
with Pool(cpu_count()) as pool:
	for i, (a, t, f, m) in enumerate(pool.imap(run, skf)):
		accuracy[i] = a
		tpr[i] = t
		fpr[i] = f
		models.append(m)

np.save("results/gist_GMM10_models", models)
np.save("results/gist_GMM10_accuracy", accuracy)
np.save("results/gist_GMM10_tpr", tpr)
np.save("results/gist_GMM10_fpr", fpr)
...
plt.savefig("results/gist_GMM10.png")
plt.show()
